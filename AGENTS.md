...
**Quote form ("cotización") — the one write path.** `POST /api/cotizacion` (`src/app/api/cotizacion/route.ts`) validates the form, writes a row to the Directus `cotizaciones` collection (with `estado: "nueva"`), and sends a notification email via `src/lib/mail.ts` using Resend after the Directus write succeeds. The public Directus role only has create on `cotizaciones`; everything else is read-only.
...
- `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `RESEND_NOTIFY_EMAIL` — Resend API key, verified sender, and quote notification recipient.
